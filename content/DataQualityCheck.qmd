...
After each drifting buoy deployment, SUD files (compressed audio file with recording metadata) were offloaded from each recording device. The SUD files were extracted using [Soundtrap Host software](https://www.oceaninstruments.co.nz/downloads/) file extraction tool (version \_\_\_). The exported files include raw audio files (.wav), recording logs, and accelerometer data.
